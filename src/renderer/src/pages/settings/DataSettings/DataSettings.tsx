...
import { safeMap } from '@renderer/utils/safeArrayUtils' // 引入安全数组操作
...
const DataSettings: FC = () => {
  ...
  return (
    <Container>
      <MenuList>
        {/* 使用安全的数组map操作 */}
        {safeMap(menuItems, (item) => (
          <ListItem
            key={item?.key || ''}
            title={t(item?.title || '')}
            active={menu === item?.key}
            onClick={() => item?.key && setMenu(item.key)}
          />
        ))}
      </MenuList>
          ...
          <>
            <SettingGroup theme={theme}>
              ...
              <SettingRow>
                ...
                <HStack style={{ gap: '5px', justifyContent: 'space-between' }}>
            ...
            <SettingGroup theme={theme}>
              ...
              <SettingRow>
                ...
                <HStack style={{ alignItems: 'center', gap: '5px' }}>
